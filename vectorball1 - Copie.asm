...
;	- stocker dans table_increments_pas_transformations : X source ,increment X, Y source, increment Y, Z source, increment Z  : tout x 2^15
...
preparation_transformation:
		...
		ldr		R11,pointeur_coordonnees_objet_source_transformation					; points de l'objet source
		ldr		R10,pointeur_coordonnees_objet_destination_transformation				; coordonnees objet destination
		
		ldr		R9,nb_etapes_transformation
		
		ldr		R0,nb_points_objet_en_cours
		
		add		R0,R0,R0,lsl #2				; 3 * nb_points_objet_en_cours
...
pointeur_position_dans_les_animations:		.long	anim2
...
table_increments_pas_transformations:		.space			64*6*4
; X,Y,Z,increment pas , tout multiplié par 2^15

buffer_coordonnees_objet_transformees:		.space			64*3*4
	.p2align 8
buffer_calcul1:
		.skip		512*64*16
buffer_calcul2:		
