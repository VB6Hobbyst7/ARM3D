...
.equ	FIQ_tempstack,						0xD0
.equ	pointeur_table_valeurs_reflet,		0xF0
...
;----------------------------------------------------------------------------------------------------------------------
RM_init:
; init table couleurs vstart vend 
	ldr		R9,pointeur_table_reflet_MEMC1
	mov		R8,#pointeur_table_valeurs_reflet
	str		R9,[R8]
...
;			STMIA     R8,{R9-R14}				; 5 28
	...
	STRNEB    R10,[R10,#0x58+2]    ;if VSync, reset T1 (latch should already have the vsyncvalue...)		: si vsync, alors on refait un GO = on remet le compteur (latch) pour le timer 1 à la valeur vsyncreturn ( mise dans les registres dans le start et  après la derniere ligne )
;
...
;
	...
	STRB      R8,[R10,#0x14+2]     ; ...and clear all IRQ_A interrupt triggers								: 1 = clear, donc ré-écrire la valeur de request efface/annule la requete d'interruption
...
;	MOV       R8,#0b00001000		; EDZ : Vsync only
	...
	STRB      R8,[R10,#0x18+2]     ;set IRQA mask to %01000000 = T1 only									: mask IRQ A : bit 6 = Timer 1, plus de Vsync

; remaskage IRQ B : clavier/keyboard
	MOV       R8,#0b10000000       ;R8,#%11000000
	STRB      R8,[R10,#0x28+2]     ;set IRQB mask to %11000000 = STx or SRx									: mask IRQ B pour clavier

; remet le compteur inter ligne pour la frequence de Timer 1 = Hsync	
	MOV       R8,#hsyncline_low			; (hsyncline AND &00FF)>>0
	STRB      R8,[R10,#0x50+2]              ;T1 low byte, +2 for write
	MOV       R8,#hsyncline_high		; (hsyncline AND &FF00)>>8
	STRB      R8,[R10,#0x54+2]              ;T1 high byte, +2 for write
