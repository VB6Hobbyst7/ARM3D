...
; check that module is a TFMX SONG : TFMX-SONG   ( ends with a space )
...
; si module tfmx compressé :	
; $1D0 = 	three longs / offsets into the file
;   - trackstep
;   - pattern data pointers
;   - macro data pointers

; si module tfmx pas compressé:
; The three longwords at $1D0 are null.  Fixed offsets of $600,$200,$400 apply.



	move.l	464(A0),D1			; = trackstart
	bne.b	OK2
	move.l	#2048,D1			; $800 ??
OK2
	cmp.w	#$8B0,14(A0)		; 08B0 ?? NTSC PAL ??
	bne.b	NoException			; BMW exception
	cmp.l	#$01F4FF00,516(A0)  ; 516 = $204 = pattern data ?
	beq.b	OK1
NoException
	moveq	#31,D2
	lea	256(A0),A1				; saute le texte : 256-319 : song start positions   320-383 : song end positions 384-447 : tempo numbers
NextPos
	moveq	#0,D3
	move.w	(A1)+,D3			; song start = Trackstep / liste des pistes / tracks du module
	cmp.w	#$01FF,D3			; 01FF = FIN 
	beq.b	SkipIt
	lsl.l	#4,D3				; song start * 16
	add.l	D1,D3				; trackstart + song start * 16
	lea	    (A0,D3.L),A2
CheckEFFE
	cmp.w	#$EFFE,(A2)+
