...
; valeurs fixes RMA
.equ	ylines,			56
.equ	vsyncreturn,	7142+(128*200)						; vsyncreturn=7168+16-1-48   +   vsyncreturn+=7
...
;
	...
	MOV       R8,#0b00001000
	STRB      R8,[R14,#0x18+2]    ;set IRQA mask to %00001000 = VSync only
...
; border	
	...
	TEQP      PC,#0x0C000003			; %000011<<26 OR %10 ;36 A4 back to IRQ mode
	MOV       R0,R0                  ;37 A8 sync IRQ registers
	SUBS      PC,R14,#4              ;38 AC return to foreground


; vsync de la ligne 200
vsync2:
	TST       R8,#0b00001000       ;retest R5 is it bit 3 = Vsync? (bit 6 = T1 trigger/HSync)
	STRNEB    R14,[R14,#0x58+2]    ;if VSync, reset T1 (latch should already have the vsyncvalue...)
	STRB      R8,[R14,#0x14+2]     ; ...and clear all IRQ_A interrupt triggers

	adr		R8,notHSync					;FNlong_adr("",0,notHSync)   ;set up VSync code after copying
	MOV     R9,#FIQ_notHSync_valeur 	;ref. works if assembling on RO3, note 'FIQ_notHSync' is 0-relative!
	STR     R8,[R9]
...
;on bascule sur vsync2
	adr		R8,vsync2					;FNlong_adr("",0,notHSync)   ;set up VSync code after copying
	MOV     R9,#FIQ_notHSync_valeur 	;ref. works if assembling on RO3, note 'FIQ_notHSync' is 0-relative!
	STR     R8,[R9]
...
; 0b 110 110 111 000 0 1 0 0 01 00 00 11 00
; 11011011100001000100001100
; 0x36E110C ?
; 0x3620000

;	ldr		R8,valeur_control_memc
;	str		R8,[R8]
...
;	ldr		R8,valeur_control_memc_normale
;	str		R8,[R8]
...
valeur_control_memc:		.long 0x36E170C
; 0b 110 110 111 000 0 1 0 1 11 00 00 11 00
;0b11011011100001011100001100
; OK default : 0x36E150C
; continous : 0x36E170C
valeur_control_memc_normale:		.long 0x36E150C
